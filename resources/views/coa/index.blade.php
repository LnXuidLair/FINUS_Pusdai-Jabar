            ...
            <div class="finus-header-gradient">
                <div class="finus-header-content">
                    <div class="finus-header-left">
                        ...
                        <div>
                            <div class="finus-header-title">Chart of Accounts</div>
                            <div class="finus-header-subtitle" id="recordCount">
                                Total: {{ is_countable($coa) ? count($coa) : 0 }} akun
                            </div>
                        </div>
                    </div>
                    <div class="d-flex align-items-center">
                        <input id="searchInput" type="search" class="form-control form-control-sm mr-2 finus-search" placeholder="Cari kode, nama, atau kelompok...">
                        <a class="btn btn-light btn-sm shadow-sm" href="{{ route('admin.coa.create') }}">
                    ...
                    <table id="searchTable" class="table table-hover table-bordered table-sm">
                        ...
                        <tbody>
                            @forelse($coa as $item)
                                <tr
                                    data-search-row
                                    data-search-start="{{ $item->kode_akun }}|{{ $item->nama_akun }}|{{ $item->label_header_akun }}" class="text-center"
                                >
                                    <td class="text-center" data-row-number>{{ $loop->iteration }}</td>
                                    <td class="font-weight-bold">{{ $item->kode_akun }}</td>
                                    <td>{{ $item->nama_akun }}</td>
                                    <td>{{ $item->label_header_akun }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.coa.edit', $item) }}" class="btn btn-outline-warning btn-sm">
