            ...
            <div class="finus-header-gradient">
                <div class="finus-header-content">
                    <div class="finus-header-left">
                        ...
                        <div>
                            <div class="finus-header-title">Penggajian</div>
                            <div class="finus-header-subtitle" id="recordCount">
                                Total: {{ is_countable($penggajians) ? count($penggajians) : 0 }} data
                            </div>
                        </div>
                    </div>

                    <div class="d-flex align-items-center">
                        <input id="searchInput" type="search" class="form-control form-control-sm mr-2 finus-search" placeholder="Cari pegawai, periode, total, atau status...">
                        <a class="btn btn-light btn-sm shadow-sm" href="{{ route('admin.penggajian.create') }}">
                            <i class="fa fa-plus"></i> Proses Gaji
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <table id="searchTable" class="table table-hover table-bordered table-sm">
                        ...
                        <tbody>
                            @forelse($penggajians as $item)
                                ...
                                <tr
                                    data-search-row
                                    data-search-start="{{ $pegawaiNama }}|{{ $item->periode }}|{{ $item->jumlah_kehadiran }}|{{ $totalFormatted }}|{{ $item->status_penggajian }}" class="text-center"
                                >
